# Alert Notification Server (ANS) application

## Overview
The ANS snippet application shows how to initialize and use the AIROC&#8482; Bluetooth&#174; Alert
Notification Server library. ANS is implemented in GAP central mode and provides ANS services to ANC devices using AIROC&#8482; Bluetooth&#174; ANS library.

## Features demonstrated
- Initialize and use the AIROC&#8482; Bluetooth&#174; ANS library
...
2. Use ClientControl to update supported alerts.
3. Pair with an ANC device.
4. Send alerts to ANC using ClientControl.
5. On power up or reset, ANS performs a Scan to connect to the Alert Notification Client (ANC), if the ANC device is advertising within the range and includes its name as "ANC" in its ADV. If an ANC device is not found with in 90 seconds, the Scan is stopped automatically. The user has to push the user button on the board to restart the Scan to reconnect to the ANC device whenever it is ready for a connection. Opening the port in the ClientControl also will initiate the scanning for ANC. The user needs to use the ClientControl ANS GUI option to generate alerts and control the alerts.
6. The application currently supports simple alerts, email and SMS/MMS alert categories, and notifies this information to ClientControl whenever the transport is connected. The user can then enable the subset of supported new alerts and unread alerts when the ANS device is not connected to the ANC device.
7. After connection to the ANC device, the ANS device sends new alerts and unread alerts to the client based on ANC configuration. New alerts and unread alerts get generated as follows:
    - ClientControl enables the GUI option to generate the alert when an ANS device has a connection with an ANC device.
    - When the user generates an Alert using the ClientControl GUI, the ANC device receives the new alert and unread alert.
    - The user can clear the alert count using the GUI Clear Alert button.
8. The application calls ANS library APIs based on user requests (i.e. on updating supported alert categories, or the generate and clear the alert requests actions). Read/Write requests received from ANC are passed to AIROC&#8482; Bluetooth&#174; ANS library to update the ANC configuration (for example to start or stop new alerts/unread alerts, and to configure to send only requested alert categories.)
