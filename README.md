...
5. On ClientControl Connect, ANS automatically performs a Scan and connects to the Alert Notification Client (ANC), if the ANC device is advertising within the range and includes its name as "ANC" in its ADV. If an ANC device is not found with in 90 seconds, the Scan is stopped automatically. The user has to push the application button on the board to restart the Scan to reconnect to the ANC device whenever it is ready for a connection. The user needs to use the ClientControl ANS GUI option to generate alerts and control the alerts.
